    ...
    if (data === targetValue) {
        ...
        for (const key in data) {
            if (data.hasOwnProperty(key)) {
                result[key] = replaceValue(data[key], targetValue, replacementValue);
            }
        }
        return result;
    } else {
    ...
    try {
        ...
        Object.keys(data).forEach(key => {
            if (key.includes('-switch')) {
                ...
                document.querySelector('#toggle-switch input').checked = isChecked;
...
function saveChanges() {
    const updatedData = {};
    const isActive = document.querySelector('#toggle-switch input').checked ? 1 : 0;
    ...
    if (typeof obj === 'string') {
        ...
        for (let key in obj) {
            if (obj.hasOwnProperty(key)) {
                newObj[key] = removeSendFromObject(obj[key]);
            }
        }
        return newObj;
    } else {
...
function loadServerDetails(serverId) {
    ...
    const serverDetails = {{ guild_details | tojson | safe
...
function start_updater() {
    document.querySelector('input[name="image-input"]').addEventListener('input', function () {
        localStorage.setItem("Image-Link", this.value);
        updateimage()
    });
}
function updateimage() {
    var bild = document.querySelector('img[alt="image-input"]');
    var linkraw = localStorage.getItem("Image-Link");
    const linkExtension = linkraw.split('.').pop().split('?')[0];
    const validExtensions = ["png", "gif", "jpg", "webp", "placehold"];
    const isValidLink = validExtensions.includes(linkExtension);
    if (!isValidLink) {
        link = "https://placehold.co/200x100?text=Sorry but your url does not work"
    } else {
        link = linkraw
    }
    bild.src = link;
