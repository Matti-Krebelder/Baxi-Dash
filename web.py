...
headers = {"Authorization": f"{config['BAXI']['baxi_info_key']}"}
baxi_data_request = requests.get(
    "https://baxi-backend.pyropixle.com/api/oauth/get/data/baxi", headers=headers
)
...
async def login():
    return redirect(
        f'{AUTH_URL}?client_id={int(baxi_data["client_id"])}&redirect_uri={baxi_data["redirect_uri"]}&response_type=code&scope=identify%20guilds'
    )
...
def log_denied_access(ip_address, user_id):
    ...
    denied_access.append({"ip_address": ip_address, "user_id": user_id})
...
async def get_servers():
    ...
    for user_guild in user_guilds:
        permissions = int(user_guild["permissions"])
        if permissions & 0x8:  # Admin permission
            for bot_guild in bot_guilds:
                if user_guild["id"] == bot_guild["id"]:
                    common_guilds.append(
                        {
                            ...
                            "icon_url": (
                                f"https://cdn.discordapp.com/icons/{user_guild['id']}/{user_guild['icon']}.png"
                                if user_guild["icon"]
                                else DEFAULT_ICON
                            ),
                        }
